...
export const handleCliArguments = async (args: string[]) => {
  ...
  for (const arg of args) {
    ...
    if (!menu || !menu.options) {
      ...
      console.log(
        `ℹ️  Help: ${
          menuStructure[currentMenu]?.helpText || "No description available."
        }`
      );
      process.exit(1);
    }
    ...
    if (!nextOption) {
      console.error(
        `❌ Error: Command "${arg}" not found in "${currentMenu}".`
      );
      console.log(
        `ℹ️  Available options: ${menu.options.map((o) => o.value).join(", ")}`
      );
      process.exit(1);
    }
    ...
    if (nextOption.action) {
      try {
        ...
        console.log(
          `ℹ️  Help: ${nextOption.helpText || "No description available."}`
        );
        process.exit(1);
      }
      process.exit(0);
  ...
  console.log(
    `ℹ️  Available options: ${menuStructure[currentMenu].options
      .map((o) => o.value)
      .join(", ")}`
  );
  process.exit(1);
